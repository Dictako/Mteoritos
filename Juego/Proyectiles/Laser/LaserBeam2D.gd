...
onready var audio_disparo:AudioStreamPlayer = $AudioDisparo
...
func set_is_casting(cast: bool) -> void:
	...
	if is_casting:
		...
		audio_disparo.play() #Activar el audio
...
func cast_beam(llamado_a_delta: float) -> void:
	if energia <= 0:
		set_is_casting(false)
		return
	
	controlar_energia(radio_desgaste * llamado_a_delta)
	...
	if is_colliding():
		...
		if get_collider().has_method("recibir_danio"):
			get_collider().recibir_danio(radio_danio * llamado_a_delta)
